...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  ({ form } = $page);
  $$unsubscribe_page();
  return `${$$result.head += `<!-- HEAD_svelte-16z9sqb_START -->${$$result.title = `<title>SnippetBox - Sign Up</title>`, ""}<!-- HEAD_svelte-16z9sqb_END -->`, ""}
...
<form method="${"POST"}" action="${"/signup"}" class="${"grid gap-5 border-t-2 border-dotted border-gray-3 pt-5"}">${form?.errors?.formErrors ? `<div class="${"mb-3 space-y-2"}">${each(form?.errors?.formErrors, (error) => {
    return `<p class="${"text-center py-5 bg-red text-white rounded-md"}">${escape(error)}</p>`;
  })}</div>` : ``}
	<div class="${"grid gap-2"}"><label for="${"name"}" class="${"text-blue"}">Name:</label>
		<input type="${"text"}" name="${"name"}" id="${"name"}"${add_attribute("value", form?.values?.name ?? "", 0)} class="${"border-2 " + escape(
    form?.errors?.fieldErrors?.name ? "border-red" : "border-gray-3",
    true
  ) + " p-4 text-gray-4 focus:outline-blue/40 outline-none rounded-md transition-all duration-200"}">
		<ul class="${"text-sm list-disc pl-4"}">${form?.errors?.fieldErrors?.name ? `${each(form?.errors?.fieldErrors?.name, (error) => {
	...
	<div class="${"border-t-2 border-dotted border-gray-3 pt-5"}"><button type="${"submit"}" class="${"inline-block rounded-md bg-green hover:bg-green/80 p-5 font-bold text-white focus:outline-blue/40 transition-all duration-200"}">Create Account</button></div></form>`;
});
export {
  Page as default
};
